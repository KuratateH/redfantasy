...
public class RedFantasy {
    ...
    public void startPhase() {
        ...
        IntStream.range(0,this.playerMonsters.length)
            ...
            .forEach(i -> System.out.print(this.monsters[this.playerMonsters[i]] + " "));
        ...
        if(d1 == 1){
            ...
            IntStream.range(0,this.playerMonsters.length)
                .filter(i -> this.playerMonsters[i] != -1)
                .forEach(i -> this.playerMonstersPoint[i] = this.playerMonstersPoint[i] / 2);
        ...
        if(d2 == 1){
            ...
            IntStream.range(0,this.playerMonsters.length)
                ...
                .forEach(i -> this.cpuMonstersPoint[i] = this.cpuMonstersPoint[i] / 2);
        }else if(d2 == 6){
            System.out.println("Critical！すべてのモンスターポイントが倍になる");
            // for(int i = 0; i < this.cpuMonsters.length; i++){
            //     if(this.cpuMonsters[i] != -1){
        ...
        IntStream.range(0,this.playerMonsters.length)
                .filter(i -> this.playerMonsters[i] != -1)
                .map(i -> this.playerMonstersPoint[i])
                .sum();
        ...
        // }
        IntStream.range(0,this. playerHistory.length)
            .filter(i -> this.playerHistory[i] == -9999)
            .findFirst()
            .ifPresent(i -> this.playerHistory[i] = this.playerHp);
        ...
        IntStream.range(0,this. cpuHistory.length)
            .filter(i -> this.cpuHistory[i] == -9999)
            .findFirst()
            .ifPresent(i -> this.cpuHistory[i] = this.cpuHp);
